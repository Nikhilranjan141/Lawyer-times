...
// import { ArrowRight, CalendarDays, Clock3, FileText, RefreshCw, Search } from "lucide-react";
...
// function ArticleImage({ item, featured = false }) {
//   const image = getArticleImage(item);
//   const className = featured ? "legal-featured-image" : "legal-card-image";

//   return (
//     <div className={className}>
//       {image ? (
//         <img src={image} alt={item.title} loading={featured ? undefined : "lazy"} />
//       ) : (
//         <div className="legal-image-placeholder" aria-label="Court document image unavailable">
//           <FileText size={34} />
//           <span>Court document</span>
//         </div>
//       )}
//     </div>
//   );
...
//         </div>
//         <h2>{capitalizeTitle(item.title)}</h2>
...
//     >
//       <ArticleImage item={item} featured />
  ...
  return (
    ...
    >
      <div className="legal-card-image">
        <img src={item.image || getArticleImage(item)} alt={item.title} loading="lazy" onError={(e) => { e.target.src = getArticleImage({...item, image: null}); }} />
      </div>
      <div className="legal-card-body">
        <div className="legal-meta-row">
          <span>{item.source}</span>
  ...
  return (
    ...
    >
      <div className="legal-featured-image">
        <img src={item.image || getArticleImage(item)} alt={item.title} onError={(e) => { e.target.src = getArticleImage({...item, image: null}); }} />
      </div>
      <div className="legal-featured-body">
        <div className="legal-meta-row">
          <span>{item.courtCategory || item.category}</span>
  ...
  const sortedItems = useMemo(() => {
    ...
    if (sort === "today") {
      filtered = filtered.filter((item) => {
        const d = new Date(item.publishDate || item.date || item.updatedAt);
        return d >= todayStart;
      });
      // If nothing today, show last 3 days
      if (!filtered.length) {
      ...
      filtered.sort((a, b) => new Date(b.publishDate || b.date || b.updatedAt) - new Date(a.publishDate || a.date || a.updatedAt));
    } else if (sort === "week") {
      filtered = filtered.filter((item) => {
        const d = new Date(item.publishDate || item.date || item.updatedAt);
        return d >= weekStart;
      ...
      filtered.sort((a, b) => (b.classificationScore || 0) - (a.classificationScore || 0));
    } else {
      // latest
      filtered.sort((a, b) => new Date(b.publishDate || b.date || b.updatedAt) - new Date(a.publishDate || a.date || a.updatedAt));
    }
    ...
    <>
      ...
      <main className="legal-intelligence-page">
        <section className="legal-page-header">
          ...
          <div className="legal-title-row">
            <div>
              <p className="legal-eyebrow">Live Legal Intelligence</p>
              <h1>{pageTitle}</h1>
              <p>{sortedItems.length} Article{sortedItems.length === 1 ? "" : "s"} • {relativeTime(meta?.updatedAt)}</p>
            </div>
            <div className="legal-header-actions">
              {headerAction ? (
                <Link to={headerAction.href} className="legal-action-link">
          ...
          <div className="legal-sort-tabs">
            {sortOptions.map((option) => (
              <button
                ...
                type="button"
                className={sort === option.value ? "active" : ""}
                onClick={() => { setSort(option.value); setVisibleCount(initialVisibleCount); }}
              >
                {option.label}
              </button>
            ))}
        ...
        {!showSkeleton && !error && !items.length ? (
          <div className="legal-state-card">
            <strong>No results found.</strong>
            <p>{sort === "today" ? "No articles published today. Try \"This Week\" or \"Latest\"." : sort === "week" ? "No articles from this week. Try \"Latest\"." : "Try a broader search or refresh once the source feeds have new approved content."}</p>
          </div>
        ) : null}
